...
const messagesWithSeparators = useMemo(() => {
  return messages.reduce((acc, message, index) => {
    ...
    if (
      index === 0 ||
      new Date(messages[index - 1].timestamp || Date.now()).toDateString() !== messageDate
    ) {
      acc.push({ type: 'separator', date: formatDateSeparator(messageDate) });
    }
    acc.push({ type: 'message', message });
  ...
  useEffect(() => {
    const fetchAlumni = async () => {
      ...
      try {
        ...
        const res = await fetch('https://alumni-student-connect-gamma.vercel.app/api/search/alumni', {
